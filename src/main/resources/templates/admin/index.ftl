...
<body class="fixed-sidebar full-height-layout gray-bg" style="overflow:hidden">
    <div id="wrapper">
		...
		<#if newList?size != 0>
            <#list newList as daresource >
                        <a href="${daresource.sourceUrl}" id="zhh" >
                            <span class="nav-label">${daresource.text}</span>
                        </a>
		<ul class="nav nav-second-level" id="lx">
		<#list daresource.nodes as xiaoresource >
            <li>
                <a class="J_menuItem" href="${xiaoresource.sourceUrl}">${xiaoresource.text}</a>
            </li>
        </#list>
        </ul>
            </#list>
   ...
   <#--     <#list resourceList as resource>
         <@shiro.hasPermission name="${resource.sourceKey}">
            <li>
                <a class="J_menuItem" href="${resource.sourceUrl}">${resource.name}</a>
            </li>
         </@shiro.hasPermission>
        </#list>-->
                    ${newList?size}
    ...
    <script src="${ctx!}/assets/js/bootstrap-treeview.js"></script>
    <!-- 自定义js -->
