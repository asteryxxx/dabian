    ...
    <script>
			...
			$("#table_list").bootstrapTable({
			    ...
			    columns: [{
                    ...
                    formatter: function (value, row, index) {
                        ...
                        operateHtml = operateHtml + '<@shiro.hasPermission name="system:role:grant:man"><button class="btn btn-info btn-xs" type="button" onclick="grantman(\''+row.id+'\')"><i class="fa fa-arrows"></i>&nbsp;&nbsp;&nbsp;分配人员</button></@shiro.hasPermission>';
                        return operateHtml;
                    }
			    }]
			});        	
        ...
        function grantman(id){
            layer.open({
                ...
                shade: false,
                area: ['893px', '600px'],
                content: '${ctx!}/admin/role/grant/man/'  + id,
                end: function(index){
                    $('#table_list').bootstrapTable("refresh");
                }
            });
