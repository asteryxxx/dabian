...
<script>
    ...
    $('#anniu').click(function(){
        ...
        $("#table_list").bootstrapTable('refresh',{
            url:　"${ctx!}/examine/account/list?formname="+encodeURIComponent(name)+"&description="+encodeURIComponent(description)+"&costtype="+encodeURIComponent(costtype),
                    silent: true,
                    query: {
                        formname : name,
                        costtype: costtype,
    ...
    $(document).ready(function () {
        ...
        $("#table_list").bootstrapTable({
            ...
            columns: [{
                ...
                field: "empty",
                formatter: function (value, row, index) {
                    var operateHtml = '<@shiro.hasPermission name="department:setting:edit"><button class="btn btn-primary btn-xs" type="button" onclick="pass(\''+row.formkey+'\')"><i class="fa fa-edit"></i>&nbsp;通过</button> &nbsp;</@shiro.hasPermission>';
                    operateHtml = operateHtml + '<@shiro.hasPermission name="department:setting:edit"><button class="btn btn-danger btn-xs" type="button" onclick="nopass(\''+row.formkey+'\')"><i class="fa fa-remove"></i>&nbsp;驳回</button> &nbsp;</@shiro.hasPermission>';
    ...
    function nopass(id){
        layer.confirm('确定驳回吗?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "${ctx!}/examine/account/nopass/" + id,
    ...
    function pass(id){
        layer.confirm('确定审核通过吗?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "${ctx!}/examine/account/pass/" + id,
                success: function(msg){
                    layer.msg(msg.message, {time: 2000},function(){
                        $('#anniu').click();
